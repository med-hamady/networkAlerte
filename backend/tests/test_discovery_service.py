...
The `db` fixture (conftest) runs on the REAL database, rolled back per test, so
all fixture IPs/MACs use synthetic ranges (10.99.*, 02:00:00:*) that never
collide with production rows. notification_service is patched (no email/HTTP).
"""
...
async def test_new_mac_free_ip_creates_lr(db, patch_notif):
    ...
    res = await discovery_service.reconcile_peers(db, parent, [
        {"mac": "02:00:00:00:00:01", "mgmt_ip": "10.99.1.5", "hostname": "Client A"},
    ])

    assert len(res.created) == 1
    assert res.created[0].rocket_id == parent.id
    assert res.created[0].ip_address == "10.99.1.5"
...
async def test_new_mac_steals_stale_churned_ip(db, patch_notif):
    ...
    stale = await _lr(db, "Old Client", "10.99.1.5", "02:00:00:00:00:02", other.id)
    parent = await _rocket(db, "PK1-OUEST", "10.99.0.1")

    res = await discovery_service.reconcile_peers(db, parent, [
        {"mac": "02:00:00:00:00:01", "mgmt_ip": "10.99.1.5", "hostname": "New Client"},
    ])
    await db.flush()
    ...
    assert len(res.created) == 1
    assert res.created[0].rocket_id == parent.id
    assert res.created[0].mac_address == "02:00:00:00:00:01"
    # The stale LR is NOT reassigned — it keeps its identity, only loses the
    # churned IP (freed to NULL).
...
async def test_mac_match_reassigns_across_rockets(db, patch_notif):
    ...
    other = await _rocket(db, "ARF1-OUEST1", "10.99.0.2")
    existing = await _lr(db, "Roaming Client", "10.99.1.9", "02:00:00:00:00:03", other.id)
    parent = await _rocket(db, "PK1-OUEST", "10.99.0.1")

    res = await discovery_service.reconcile_peers(db, parent, [
        {"mac": "02:00:00:00:00:03", "mgmt_ip": "10.99.1.9", "hostname": "Roaming Client"},
    ])
...
async def test_legacy_macless_lr_adopted_not_duplicated(db, patch_notif):
    ...
    legacy = await _lr(db, "Legacy", "10.99.1.55", None, parent.id)

    res = await discovery_service.reconcile_peers(db, parent, [
        {"mac": "02:00:00:00:00:04", "mgmt_ip": "10.99.1.55", "hostname": "Legacy"},
    ])

    assert res.created == []
    adopted = await db.get(Lr, legacy.id)
    assert adopted.mac_address == "02:00:00:00:00:04"
...
async def test_no_mac_ip_fallback_scoped_to_same_rocket(db, patch_notif):
    ...
    parent = await _rocket(db, "LTU-A", "10.99.0.1", tech="ltu")
    same = await _lr(db, "Legacy A", "10.99.1.26", None, parent.id)
    other = await _rocket(db, "LTU-B", "10.99.0.2", tech="ltu")
    await _lr(db, "Legacy B", "10.99.2.99", None, other.id)

    # Same-Rocket, no MAC → matched (no duplicate created).
    res = await discovery_service.reconcile_peers(db, parent, [
        {"mac": None, "mgmt_ip": "10.99.1.26", "hostname": "Legacy A"},
    ])
    assert res.created == []
...
async def test_no_mac_foreign_ip_creates_not_steals(db, patch_notif):
    ...
    other = await _rocket(db, "LTU-B", "10.99.0.2", tech="ltu")
    foreign = await _lr(db, "Foreign", "10.99.2.4", None, other.id)
    parent = await _rocket(db, "LTU-A", "10.99.0.1", tech="ltu")

    res = await discovery_service.reconcile_peers(db, parent, [
        {"mac": None, "mgmt_ip": "10.99.2.4", "hostname": "Foreign"},
    ])
    await db.flush()
