...
| `lr_topology_check_job` | 60 min | Détecte mode routeur vs bridge sur chaque LR **LTU** (via SSH) ; ouvre un incident `lr_bridge_mode_misconfig` (warning) si bridge → le blocage n'est pas opérationnel sur ce LR tant qu'il n'est pas repassé en routeur. **Les LR airMAX en sont exclus** : leur netrole est lu gratuitement chaque 60 s dans `airos_api_poll_job`. Logique d'incident factorisée dans `_apply_lr_topology` (partagée SSH + airOS) |
