...
⚠️ `POST /api/v1/uisp/assign` vit dans `uisp_assign.py`, PAS ici : elle est
consommée par un tiers et porte sa propre clé cloisonnée. Le sync, qui réécrit
l'inventaire, doit rester sous la clé maîtresse — les réunir dans un même router
donnerait au tiers l'accès au sync (une dépendance de router est additive et ne
se surcharge pas par route).
"""
