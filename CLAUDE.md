...
| Point-à-Point | `/site-links` | Sous-page de **Liaisons** dans le menu (menu volant, avec « Liaisons clients » = `/lr-health`), même droit `lr_health.view`. Les backhauls P2P (AF60 / airMAX) sous leur plancher de capacité, **dessinés pylône ↔ pylône** : un F60 sur chaque pylône, le faisceau rouge sous le plancher (vert sinon), la capacité du bout le plus dégradé au centre, la mesure de chaque extrémité à l'extérieur de son pylône. ⚠️ Les deux bouts d'un même lien sont **appariés par le NOM** (`F60 PK1-CT2` + `F60 CT2-PK1`) faute de site/pair dans la réponse — un nom hors convention est dessiné seul, jamais rattaché à un faux pair. Était un onglet de `/lr-health` jusqu'au 2026-09-19. Source : `/lr-health/site-links` |
