"""/access: « hors ligne » = LR non vu par UISP depuis > 1 semaine

La stat + le filtre « hors ligne » de /access comptaient tout LR dont
uisp_status != 'active'. L'opérateur ne veut y voir que les clients déconnectés
depuis **plus d'une semaine**, basé sur l'API UISP (le dernier contact connu =
`uisp_last_seen`). On ajoute `long_offline = uisp_last_seen < now() - 7 jours` et
on l'utilise pour la stat `disconnected` et le filtre `disconnected`. Le reste
(mode UISP, reachable pour le tri/bouton) est inchangé.
...
# long_offline = vu par UISP il y a plus d'une semaine (NULL → non compté).
_ACCESS_NEW = """
...
WITH all_lrs AS (
    SELECT d.id, d.name, d.ip_address,
           ...
           (l.uisp_last_seen < now() - interval '7 days') AS long_offline
...
# Previous (h5c6) definition: disconnected = NOT reachable (any disconnected LR).
_ACCESS_OLD = """
CREATE OR REPLACE FUNCTION fn_access_clients(p_search text, p_filter text) RETURNS jsonb
LANGUAGE sql STABLE AS $$
WITH all_lrs AS (
