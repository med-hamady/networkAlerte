...
| GET | `/api/v1/fai/verify` | FAI | **Contrôle pré-vol d'un LR par MAC** pour le système de paiement (lecture seule, tout lu en base rafraîchie par les sondes). 4 contrôles : **existe** (sinon `KO`, `name=null`, **200 pas 404**), **ssh_active** (`lrs.ssh_status=="ok"`), **password_valid** (ssh_active ET `ssh_password==fai_expected_lr_ssh_password`, défaut `A2HQ@87654321` — l'auto-repair promeut le mdp qui marche sur la fiche, donc les deux prouvent que c'est lui qui ouvre la session), **router_mode** (`topology_mode=="router"`). Renvoie `ok`/`status` (`OK`/`KO`) + `name` + `reason` (résumé des échecs) + `checks{}` (détail par contrôle) + brut `ssh_status`/`topology_mode`/`ssh_checked_at`. 400 si MAC mal formée |
