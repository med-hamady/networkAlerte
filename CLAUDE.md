...
│   ├── lr_latency_sample.py # Historique de latence LR → Internet en buckets 5 min (1 ligne/(device_id, bucket_start), avg/min/max/sample_count). Table DÉDIÉE et pas `device_metrics` : à ~600 LR × 1 relevé/min, empiler ferait ~860k lignes/jour (cf. l'épisode de bloat) — le bucket 5 min ramène à ~5,2 M lignes sur 30 j
...
│   ├── lr_latency_history_service.py # Historique de latence LR → Internet (table `lr_latency_samples`) — alimente le graphe de la fiche équipement. **ÉCRITURE** : `record_sample` replie chaque RTT de la sonde dans un **bucket 5 min** par upsert (moyenne glissante recalculée EN SQL sur la ligne existante + `least`/`greatest` sur min/max) → 1 ligne/(LR, 5 min), pas 1 ligne/cycle. **LECTURE** : `get_history` sert 24h à la résolution native et re-binne les fenêtres larges via `date_bin` (moyenne **pondérée par `sample_count`** — une moyenne de moyennes sur-pondérerait un bucket à 1 seul relevé). **Trous NON comblés** : un bucket sans relevé est absent, jamais ramené à 0
...
| `LR_LATENCY_HISTORY_RETENTION_DAYS` | Rétention batchée de `lr_latency_samples` (historique de latence par client, défaut **30** j ; `lr_latency_retention_job`). ~600 LR × 288 buckets/j ≈ 173k lignes/j → ~5,2 M lignes en régime permanent |
| `LR_LATENCY_HISTORY_RETENTION_INTERVAL_MINUTES` | Intervalle du `lr_latency_retention_job` (défaut 360 = 6 h) |
...
| `lr_internet_probe_job` | 60s | SSH sur **chaque LR** avec credentials → `ping -c 5` vers `LR_LATENCY_TARGET` (8.8.8.8). Détecte à la fois la perte de transit (`lr_no_transit` après 2 cycles KO) et la dégradation de latence (`lr_latency_high` si avg ≥ 100 ms sur 3 cycles). Le RTT est persisté **deux fois** : en `device_metrics.lr_latency_ms` (collapse = dernière valeur, lu par `/lr-health`) **et** dans `lr_latency_samples` via `lr_latency_history_service.record_sample` (bucket 5 min = la SÉRIE du graphe client). Pas de transit → **rien** n'est écrit dans l'historique et la métrique collapse est purgée → le graphe montre un trou, pas un 0 |
...
| `lr_latency_retention_job` | `LR_LATENCY_HISTORY_RETENTION_INTERVAL_MINUTES` (6 h) | Purge `lr_latency_samples` plus vieux que `LR_LATENCY_HISTORY_RETENTION_DAYS` (30 j) en **batches** (`DELETE … WHERE id IN (SELECT id … LIMIT n)`, jamais une grosse transaction). Groupe scheduler **fast**. Même forme que `traffic_stats_retention_job` |
...
Tout le reste est collapsé (latest-only). **`lr_latency_ms` reste collapsé** malgré le graphe de latence client : cette série-là ne vit PAS dans `device_metrics` mais dans la table dédiée `lr_latency_samples` (buckets 5 min, cf. `lr_latency_history_service`) — c'est exactement pour ne pas rouvrir le robinet à ~860k lignes/jour dans une table déjà sujette au bloat. **Les métriques radio (`signal_dbm`, `cinr_db`, `ccq_pct`, `link_potential_pct`, `total_capacity_mbps`, `local/remote_rx_rate_idx`) sont aussi collapsées** depuis que les 2 sections 30 j du rapport (« santé liens clients » + « qualité radio ») ont été retirées : plus aucun lecteur d'historique radio. La page « Liaisons clients » tourne en LIVE (`get_live_link_health`, fetch direct LTU/airOS), pas sur `device_metrics`. La matview `lr_health_metric_stats_30d` + son job de refresh ont été supprimés (migration `x5d6e7f8a9b0`). L'alert engine lit ses baselines (EMA throughput, deltas d'erreurs) depuis `AlertState`, **jamais** depuis `device_metrics` → collapser ne casse aucune alerte. Sans cette politique, un seul UISP Power empilait ~25 métriques toutes les 30 s (~70k lignes/jour) que rien ne relit.
...
| GET | `/api/v1/devices/{id}/latency-history` | Oui | **Historique de latence d'un client** (graphe « Plus d'infos » de la fiche). Soit `?period=24h\|7d\|30d`, soit `?start=&end=` (`YYYY-MM-DD` UTC, **fin incluse**, les deux ensemble sinon 422 — même convention que `/clients/consumption`) ; la plage l'emporte sur `period`. Renvoie `points[]` (avg/min/max/sample_count par bucket) + `bin_seconds` + `threshold_ms` (seuil effectif, pour que le graphe trace la même ligne que l'alerting). Un device non-LR renvoie une série **vide** (pas une erreur) : seuls les LR sont sondés. `lr_latency_history_service` |
...
| Page | Chemin | Contenu |
|---|---|---|
| Devices | `/devices` | Liste avec statut, dernière vue, métriques, modal détail. Sur un **LR**, la fiche expose un bouton **« Plus d'infos — graphe de latence »** (`LatencyHistoryModal`) : courbe SVG de la latence Internet du client sur 24h/7j/30j ou une plage de dates, avec bande min/max (garde visible un pic court noyé par la moyenne 5 min), ligne de seuil critique, survol détaillé et chiffres clés. **Les trous = périodes sans transit**, pas des 0. Source : `/devices/{id}/latency-history` |
