...
export const endpoints = {
  ...
  // même convention que /clients).
  deviceMetricHistory: (id: number, metric: string, period: '24h' | '7d' | '30d') =>
    `${API_BASE}/devices/${id}/metric-history?metric=${metric}&period=${period}`,
  deviceMetricHistoryRange: (id: number, metric: string, start: string, end: string) =>
    `${API_BASE}/devices/${id}/metric-history?metric=${metric}&start=${start}&end=${end}`,
