...
| `API_KEY` | **Clé MAÎTRESSE** (header `X-API-Key`) — ouvre toute l'API. C'est l'auth d'origine, machine-à-machine, conçue quand il n'y avait qu'un seul appelant : nous. ⚠️ **Le dashboard ne s'en sert PAS** : son proxy (`app/api/proxy/[...path]/route.ts`) authentifie **par cookie de session** et refuse les en-têtes d'auth venus du navigateur — c'est pourquoi `API_KEY` n'est **plus** passée au conteneur frontend (retirée le 2026-09-10 ; le commentaire de `lib/api.ts` qui prétendait l'inverse était périmé). Ne restent que les appels **directs** à `/api/v1/` par l'outillage d'exploitation. **Un tiers ne doit jamais la recevoir** : c'est à ça que servent les 5 clés cloisonnées. ⚠️ **Chaque usage est journalisé en WARNING** (`master API_KEY used on <méthode> <chemin> from <ip>`, `_log_master_key_use` dans `deps.py` — chemin seul, jamais la clé ni la query string) : sans ça « qui en dépend encore ? » est sans réponse, `log_format main` de nginx n'enregistrant aucun en-tête — et la **rotation décidée le 2026-08-11 reste inexécutable**. `dc logs backend \| grep "master API_KEY"` avant de tourner. Verrouillé par `tests/test_master_key_audit.py` |
...
| `OUT_OF_SUPERVISION_DAYS` | Seuil « **hors supervision** » pour un LR (défaut **60** j ≈ 2 mois depuis le 2026-09-22, 7 j avant — à 7 j une antenne éteinte une semaine sortait déjà des accès actifs). Un LR **sans IP** est hors du `_ping_sweep` (qui filtre `ip_address IS NOT NULL`) → plus rien ne mesure son état ; si en plus **UISP** ne l'a pas vu depuis ce délai (`uisp_last_seen` nul = silence), **les deux sources se taisent** : ni panne constatée, ni accès actif. Exclu du décompte « Accès actif » de `/access` + badge ambre **HORS SUPERVISION** (au lieu du rouge « INCONNU », lu à tort comme une panne — 124 lignes sur ~1000 en prod le 2026-07-22). **Aucune suppression** : la découverte les récupère seules dès qu'un AP les rapporte avec une IP du plan. Règle écrite 2× : `schemas/device.is_out_of_supervision` (fiche) et `fn_access_clients` (SQL, migration `cc3d4e5f6a7b`) — les garder d'accord |
...
| `UISP_ASSIGN_API_KEY` | **Clé API cloisonnée à la seule route `POST /uisp/assign`**, tenue par le système de paiement qui adopte les équipements installés. ⚠️ Raison d'être : `/uisp/assign` est servie sur la **VIP publique `.229`**, qui fronte l'**API ENTIÈRE** (contrairement au listener `.233`, restreint à `/fai`) — sans clé dédiée, faire consommer cette route par un tiers oblige à lui confier `API_KEY`, donc `DELETE /devices/{id}` et `/uisp/sync` par la même occasion (situation constatée le **2026-08-11** : l'équipe paiement détenait la clé maîtresse). Ne **retombe pas** sur l'auth `/fai` : bloquer un abonné et écrire dans le contrôleur sont deux pouvoirs distincts. Vide = repli sur l'auth normale (`API_KEY`/session), c.-à-d. ce qu'on veut éviter. Router séparé `endpoints/uisp_assign.py` — une dépendance de router étant **additive et non surchargeable par route**, c'est la seule façon de cloisonner sans ouvrir `/uisp/sync`. ⚠️ **Les 6 clés doivent porter des valeurs DISTINCTES** : `_validate_production_secrets` **refuse de démarrer** si deux d'entre elles partagent une valeur — une duplication annulerait le cloisonnement en silence (qui tient l'une tient l'autre) et aucun test d'isolation ne la verrait, ces tests comparant des comparateurs et non les valeurs déployées. Verrouillé par `tests/test_uisp_assign_scoped_key.py` |
| `CONTENT_BLOCK_API_KEY` | **Clé API cloisonnée aux seules routes `/content-filter`** (filtre de contenu par plateforme, indexé par MAC), tenue par le système tiers qui pilote les options de filtrage vendues à l'abonné. ⚠️ Ne **retombe pas** sur l'auth `/fai` : filtrer TikTok chez un abonné et lui **couper la ligne** sont deux pouvoirs distincts, tenus par deux systèmes distincts — partager une clé rendrait chacun capable du travail de l'autre. Vide = repli sur l'auth normale (`API_KEY`/session), c.-à-d. ce qu'on veut éviter. ⚠️ **Les 6 clés doivent porter des valeurs DISTINCTES** (cf. `UISP_ASSIGN_API_KEY`). Verrouillé par `tests/test_content_filter_api.py` |
| `CLIENT_SIGNAL_API_KEY` | **Clé API cloisonnée à la seule route `GET /client-signal`** (qualité du signal et de la latence d'un abonné, par MAC de son LR), tenue par le système tiers qui interroge la qualité de service. ⚠️ Ne **retombe** ni sur l'auth `/fai` ni sur celle de `/content-filter` : **LIRE** la qualité d'un lien et **AGIR** sur l'abonné (le couper, filtrer son trafic) sont des pouvoirs distincts, tenus par des systèmes distincts. ⚠️ Même raison d'être que `UISP_ASSIGN_API_KEY` : la route est servie sur la **VIP `.229`** qui fronte l'**API ENTIÈRE**, donc sans clé dédiée la faire consommer par un tiers oblige à lui confier `API_KEY` — `DELETE /devices/{id}` et `/uisp/sync` compris. ⚠️ **Pas de router séparé** ici (contrairement à `/fai/verify` et `/uisp/assign`) parce que `/client-signal` a son **propre préfixe** et ne porte qu'**une** route : aucune voisine dont la clé hériterait. Corollaire à ne pas rater — une dépendance de router étant **additive**, toute route ajoutée dans `endpoints/client_signal.py` s'ouvrirait à cette clé **en silence** ; elle doit aller ailleurs. Vide = repli sur l'auth normale (`API_KEY`/session), c.-à-d. ce qu'on veut éviter. ⚠️ **Les 6 clés doivent porter des valeurs DISTINCTES** (cf. `UISP_ASSIGN_API_KEY`). Verrouillé par `tests/test_client_signal_scoped_key.py` |
...
silence** : refus 409 `device_already_assigned` avec l'id **et le nom CRM** du
détenteur actuel, `force: true` pour passer outre (`reassign`, l'ancien nom, reste
accepté). Déplacer un CPE est légitime (matériel récupéré et réinstallé ailleurs)
...
la clé (sans elle il ne se déclare jamais → rien à associer) ; puis attendre qu'il
se déclare (60 s au plus, dans le budget global de l'appel) et l'associer dans le
même appel. `pending_registration` + `retry_after_seconds` seulement s'il ne s'est
pas déclaré à temps.
...
⚠️ **La clause `CASE` du rollup doit rester IDENTIQUE** à celle de la requête
live (delta négatif = compteur remis à zéro → 0 ; delta aberrant → 0). Une
divergence ferait qu'une journée archivée ne correspond plus à ce que la
journée en cours affiche pour le même client — un écart invisible.
...
##### Les deux matviews ont été SUPPRIMÉES (2026-09-25)

`client_consumption_30d` et `client_consumption_7d`, avec leurs deux REFRESH
nocturnes (migration `i5d6e7f8a9b0`). Deux raisons indépendantes :

1. **Elles interdisaient la rétention.** Chaque REFRESH relit la fenêtre
   entière de `device_metrics` — 30 jours pour l'une. Purger au-delà de 7 jours
   aurait fait afficher à `/clients` une consommation de 30 jours **calculée
   sur 7** : un chiffre faux, sans la moindre erreur pour le dire.
...
d'une période. Les trois garde-fous sont décrits dans la ligne
`DEVICE_METRICS_RETENTION_*` du tableau des variables d'environnement.
...
| Charge AP | ~~`rocket_client_overload`~~ | **SUPPRIMÉ le 2026-09-25** (migration `j6e7f8a9b0c1`). La règle tournait encore mais son incident était supprimé depuis le 2026-06-25 : rien n'en sortait. La saturation d'un Rocket se lit sur **`/capacity`** et dans le **rapport PDF quotidien** : clients **installés** (roster UISP, `uisp_ap_name`) **moins les hors supervision** (même règle que `/access`, renvoyés à part dans `out_of_supervision_clients`) contre la capacité max. Capacité max = **formule** `_rocket_overload_threshold` (base LTU 15 / airMAX 10 à 10 MHz + `rocket_overload_clients_per_10mhz` par +10 MHz, réglable page Seuils) ou override `rockets.max_clients_override` |
...
| GET | `/api/v1/network-uptime/downtime-log` | **uptime/dashboard/reports/sites** | **L'historique DATÉ des pannes** : pour `?start=&end=` (ISO-8601), chaque équipement d'infra tombé au moins une fois, et pour chacun ses épisodes (`started_at`, `ended_at`, `duration_seconds`, `is_ongoing`) + `site`, `availability_pct`, `longest_episode_seconds`. ⚠️ **Fusion du flapping** : deux coupures séparées de moins de `merge_gap_seconds` (défaut **300 s**) comptent pour UN épisode marqué `flap_count > 1` (et `flaps[]` porte les cycles bruts) — sans quoi un lien qui bat 50× dans l'heure produirait 50 pannes. ⚠️ **Les LR abonnés en sont absents par nature** : un LR qui tombe est une panne côté client et n'ouvre aucun incident, donc rien ne permet de la reconstituer. Ne peut lire QUE les types de `AVAILABILITY_ALERT_TYPES`, seuls incidents conservés après résolution. `network_uptime_service`. Voir **Journal des coupures** |
| GET | `/api/v1/network-uptime/window-report/pdf` | **uptime/dashboard/reports/sites** | **Rapport PDF des coupures de TOUS les sites sur une TRANCHE HORAIRE** répétée chaque jour de la période (bouton « Rapport PDF par tranche horaire » de `/downtime-log`). `?start=&end=` (`YYYY-MM-DD` UTC, fin **incluse**, **92 j au plus**) + `?from_hour=&to_hour=` (0-23, défaut **0 → 8**) ; `to_hour < from_hour` = la tranche **enjambe minuit** (22 → 6), `from_hour == to_hour` = 422. ⚠️ **Règle de comptage (décision opérateur 2026-09-24)** : une coupure n'est comptée que si elle **COMMENCE dans la tranche**, et sa durée comptée s'arrête à la fin de la tranche — une coupure de 23 h 30 n'entre pas dans 00 h → 08 h. Un site **déjà coupé à l'ouverture** n'est pas compté. ⚠️ La fusion anti-flapping (5 min) se fait **à l'intérieur** de la tranche, sinon une coupure de 00 h 02 serait avalée par un épisode ouvert à 23 h 58. Mêmes règles que le journal : panne de site = coupure de son **switch** (dispo = pire switch), **tous** les sites à switch listés même sans panne. ⚠️ **PDF volontairement dépouillé** (décisions opérateur 2026-09-24) : **switches seulement**, un seul tableau **Site / Pannes / Temps coupé** sous un résumé — ni détail tranche par tranche, ni disponibilité, ni tranches touchées, ni annexe, ni texte d'explication. La fusion à 5 min s'applique toujours. Tranche en cours ramenée à maintenant. `night_outage_report_service`, verrouillé par `tests/test_night_outage_report.py` |
| GET | `/api/v1/network-uptime/site-summary` | **uptime/dashboard/reports/sites** | Cumuls **par site** (nombre de pannes + temps de coupure) pour les graphes « Pannes par site » du tableau de bord et de `/reports`. Tout en SQL (`fn_site_outage_summary`). ⚠️ Applique la **même règle** que la page : les pannes d'un site sont celles de son **switch** ; `extra_devices` porte le surplus des équipements tombés au-delà de lui |
...
| GET | `/api/v1/network-capacity` | Oui | Capacité clients : par famille (LTU/airMAX) et par site, clients **installés supervisés** (roster UISP, **hors supervision exclus** — nombre exclu dans `out_of_supervision_clients` par Rocket) vs capacité max. Rockets sans largeur connue exclus des totaux (`unknown`). `network_capacity_service`. Inclut aussi la clé **`infra`** (`site_infra_service.get_site_infra_capacity`) : budget d'équipements infra par site (Rockets+AF60+PTP) vs `SITE_INFRA_MAX`, avec marge `remaining` signée |
...
| GET | `/api/v1/client-signal` | **Signal** | **Qualité de connexion d'un abonné par MAC** de son LR, pour un système tiers. Deux verdicts de nature DIFFÉRENTE dans la même réponse, et c'est le point à ne pas confondre : le **signal** (`quality` : `excellent` ≥ -65 / `bien` / `moyen` / `faible` < -80 dBm) est lu de la **dernière valeur en base** — instantané mais **daté**, d'où `measured_at` — tandis que la **latence** (`latency_quality` : `excellent` < 80 ms / `tres_bien` / `bien` / `mauvaise` / `catastrophique` ≥ 150) est **mesurée EN DIRECT à l'appel** (session SSH sur le LR + 5 pings de 56 o vers `lr_latency_target`). ⚠️ **Appel lent — 6-15 s**, davantage sur un lien en perte : prévoir le timeout client, et **séquencer** un lot plutôt que de le paralléliser (la file SSH est partagée avec les sondes de supervision). ⚠️ Mesure impossible ⇒ `indetermine` **avec la raison** dans `latency_message`, **jamais 0 ni valeur inventée** — un tiers doit pouvoir distinguer « lien mauvais » de « rien mesuré ». **Clé API DÉDIÉE `CLIENT_SIGNAL_API_KEY`** (`require_client_signal_client`) : scellée à cette seule route, **sans repli** sur l'auth `/fai` ni sur `/content-filter`. ⚠️ **Location nginx dédiée** (`location ^~ /api/v1/client-signal`) : `proxy_read_timeout` **120 s** au lieu des 30 s généraux — à 30 s l'appelant reçoit un 504 sur une mesure RÉELLEMENT faite (piège déjà corrigé sur `/fai`, `/uisp/assign` puis `/content-filter`). Zone de débit `client_signal` 120 r/min. Servie sur la VIP `.229`, **pas** sur le listener `.233`. 400 MAC mal formée · 404 aucun LR avec cette MAC. Doc d'intégration : `docs/api-client-signal.md`. `client_signal_service` |
| GET | `/api/v1/client-signal/history` | **Signal** | **Courbes des 7 derniers jours d'un abonné par MAC** : `lr_latency_ms`, `link_potential_pct`, `total_capacity_mbps`, `dl_throughput_mbps`, points de 30 min `{t, avg, min, max}` + seuil effectif (même service et mêmes seuils que les graphes « Plus d'infos » de la fiche — `lr_metric_history_service`). ⚠️ **Endpoint SÉPARÉ de `/client-signal` exprès** : ici tout est lu en base (quelques ms), là on ouvre une session SSH (6-15 s) — un tiers qui ne veut que les graphes ne doit ni attendre le SSH ni charger la file SSH partagée avec nos sondes. Trous **non comblés**, les 4 courbes **toujours présentes** (vides si aucun relevé, ex. potentiel d'un M5). Même clé `CLIENT_SIGNAL_API_KEY` : **deuxième route voulue** du router `client_signal.py` (lecture de la qualité d'un lien, même pouvoir) — la liste des routes que cette clé ouvre est verrouillée par `_ROUTES_OPEN_TO_THE_KEY` dans `tests/test_client_signal_scoped_key.py`. Couverte par la location nginx `^~ /api/v1/client-signal` existante |
...
| Accès clients | `/access` | Table des LR abonnés (source UISP). Chaque ligne porte l'**id du client CRM** (`lrs.uisp_crm_client_id`, colonne « ID CRM ») **seulement s'il existe** — un équipement non rattaché dans UISP n'en affiche aucun ; la recherche accepte aussi cet id (égalité exacte, migration `g3b4c5d6e7f8`). ⚠️ Les **compteurs du parc** (total / actifs / bloqués / bridge, et les badges des onglets) sont soumis au droit `fai.stats` et **retirés de la réponse** quand il manque — `stats` vaut alors `null`, jamais des zéros. Le filtrage, la recherche et le tri restent entiers. Voir **Profils d'accès — qui voit quoi**. Filtres dont **« Hors supervision »** : LR sans IP **et** non vu par UISP depuis `OUT_OF_SUPERVISION_DAYS` — badge ambre, **exclu du compteur « Accès actif »** (la tuile indique combien sont exclus). Distinct de « Hors ligne > 1 mois » (`long_offline`, absence prolongée vue par UISP) : ici c'est une absence de **mesure**, pas une absence constatée |
| Point-à-Point | `/site-links` | Sous-page de **Liaisons** dans le menu (menu volant, avec « Liaisons clients » = `/lr-health`), même droit `lr_health.view`. Les backhauls P2P (AF60 / airMAX) sous leur plancher de capacité, **dessinés pylône ↔ pylône** : un F60 sur chaque pylône, le faisceau rouge sous le plancher (vert sinon), la capacité du bout le plus dégradé au centre, la mesure de chaque extrémité à l'extérieur de son pylône. Les deux bouts sont **appariés par le BACKEND depuis le câblage** (`site_links`, par MAC — la même source que `/topology`), plus d'après le nom (2026-09-22). Chaque bout porte la **raison** de son état (`measured` / `no_data` / `down` / `unknown` / `unsupervised` / `uncabled`), qui remplace l'ancien « extrémité non listée » recouvrant cinq cas. Seuls les bouts **en ligne ET mesurés** entrent dans le verdict, la liaison vaut le **pire** d'entre eux ; aucun bout mesuré ⇒ liaison **non évaluée** (nommée dans une liste dépliable), jamais « saine ». Une radio P2P absente du câblage n'est pas escamotée (autre bout `uncabled`). Plancher AF60 = **1950 Mb/s sur la MOYENNE** des deux sens depuis le 2026-09-22 (il était à 975) — l'alerte WhatsApp `af60_link_substandard` garde son propre réglage. Logique pure : `lr_health_service.build_site_link_pairs`, verrouillée par `tests/test_site_link_pairs.py` Était un onglet de `/lr-health` jusqu'au 2026-09-19. Source : `/lr-health/site-links` |
| Journal des coupures | `/downtime-log` | **L'historique daté des pannes** — « AT1 : tombé 16:04, revenu 18:12, 2h08 ». Sous **Supervision**, droit `uptime.view`. Fenêtre 24 h / 7 j / 30 j (défaut **7 j**) ou plage de dates ; la fenêtre d'un raccourci **glisse avec l'horloge** (battement 60 s), sinon une panne survenue après l'ouverture de l'onglet ne rentrerait jamais dans la borne haute. Une carte par **site**, la plus touchée d'abord, avec ses épisodes datés, le badge **instable · N cycles** sur un épisode fusionné et **toujours coupé** sur une panne en cours. ⚠️ **Une panne de site est une coupure de son SWITCH** — la règle de `fn_site_outage_summary` (migration `r6b7c8d9e0f1`), reprise telle quelle pour que la page et les graphes du tableau de bord ne puissent pas se contredire ; un site à deux switches vaut le **pire** des deux, jamais leur moyenne. ⚠️ **Les équipements NON-switch tombés sont REPLIÉS, jamais tus** : ils ne comptent pas dans les chiffres (qui restent ceux des switches) mais sont nommés sous le journal — les taire ferait lire « aucune panne » là où un secteur entier était mort, un négatif faux. ⚠️ **Aucun abonné (LR)** : voir l'endpoint. Source : `/network-uptime/downtime-log` |
