...
| `device_ping_job` | 30s | Ping ICMP tous les devices. **Ping multi-paquets** (`ping -c 3`, joignable si ≥1 réponse) pour tolérer l'ICMP rate-limité des radios Ubiquiti. **Statut `down` seulement au seuil anti-flap** (`ping_down_threshold`=3 cycles), jamais sur un seul paquet perdu — sinon un Rocket qui route le trafic client + répond à son API s'affichait « HORS LIGNE » et sortait des polls API/SNMP (qui filtrent `status=up`). L'incident `*_down` suit le même seuil. |
