"""/access : exposer l'id du client CRM de chaque abonné

Ajoute `uisp_crm_client_id` (colonne posée par `f2a3b4c5d6e7`, écrite par le
sync UISP des stations et par `/uisp/assign`) aux lignes de `fn_access_clients`,
pour l'afficher sous le nom du client. NULL = pas rattaché à un client CRM dans
UISP : la page n'affiche alors rien, elle n'invente pas d'id.

La recherche accepte aussi l'id CRM exact (égalité, pas ILIKE : « 12 » ne doit
pas ramener les clients 120, 312, 1200…).

Rien d'autre ne change : stats, filtres et tri sont repris à l'identique.
...
filtered AS (
    SELECT * FROM all_lrs
     WHERE (p_search IS NULL OR p_search = ''
            ...
            OR uisp_crm_client_id = p_search)
...
# Corps de la révision précédente (w5j6k7l8m9n0), sans l'id CRM.
_PREV_BODY = """
CREATE OR REPLACE FUNCTION fn_access_clients(
    p_search text,
    p_filter text,
