...
- [x] **Notifications — WhatsApp (Ultramsg) remplace l'email** — depuis le 2026-06-11 le canal résolu depuis l'env est **WhatsApp** (`WHATSAPP_ENABLED` + `WHATSAPP_INSTANCE_ID` + `WHATSAPP_TOKEN` + `WHATSAPP_GROUP_ID`) : tout le pipeline d'incidents (immédiat + digest + sécurité) part vers le **groupe WhatsApp** via `whatsapp_service` (`POST /{instance}/messages/chat` Ultramsg). `email_service` n'est plus dans le pipeline — gardé **uniquement** pour le diagnostic `POST /api/v1/system/test-email`. Diagnostic WhatsApp : `POST /api/v1/system/test-whatsapp`. Le registre `alert_policy.py` reste interne ; ses jeux de canaux pointent tous sur `AlertChannel.WHATSAPP`. **Restriction (2026-06-11)** : WhatsApp ne pousse QUE les alert_types de la liste blanche `WHATSAPP_ALERT_TYPES` (`alert_constants`) — les **5 anomalies demandées** : `switch_port_speed_low`+`switch_port_down`, `device_flapping`, `battery_low_warning`, `af60_link_substandard`+`af60_link_down`, plus la latence réseau (envoi direct du `network_latency_aggregate_job`). Toute autre anomalie (équipement injoignable, qualité radio, voltage, coupure secteur, **sécurité**, découverte LR…) ouvre/résout son incident en DB mais **n'est notifiée nulle part**. Le chokepoint est `notification_service._dispatch` (+ `notify_security_event` + collecte du digest). (Historique 2026-06-09 : avant WhatsApp, l'email était env-only `SMTP_ENABLED`+`NOTIFICATION_EMAILS` ; `notification_channels`/`/notification-channels` et `/alert-policies` supprimés.)
